...
public class ExporterServiceImplTest {
	...
	public void testGetCountryCrisisHistoryData() throws FileNotFoundException, IOException {
...

		{
			...
			Assert.assertEquals("", reportRowCH070.getMetadata().get(MetadataName.DATASET_SUMMARY));
...

		{
			...
			Assert.assertEquals("Extracted from 1st hand sources", reportRowCH080.getMetadata().get(MetadataName.DATASET_SUMMARY));
			Assert.assertEquals("http://mdgs.un.org/unsd/mdg/Metadata.aspx?IndicatorId=0&SeriesId=589", reportRowCH080.getMetadata().get(MetadataName.MORE_INFO));
	...
	public void testGetCountry5YearsData() throws FileNotFoundException, IOException {
		...
		Assert.assertEquals("Average for 5 years", reportRow.getMetadata().get(MetadataName.DATASET_SUMMARY));
