...
public class RegionDictionaryDAOImplTest {
	...
	public void testRegionDictionaryDAO() {
        ...
        ResourceConfiguration configuration = resourceConfigurationDAO.createResourceConfiguration("Test Config", null, null);


        // Add some RegionDictionaries
		regionDictionaryDAO.createRegionDictionary("Fooléndia", "Importer1", foolandia, configuration);
		regionDictionaryDAO.createRegionDictionary("Fooléndia", "Importer2", barlandia, configuration); // in a different importer, the same string could mean
																						// something different
		regionDictionaryDAO.createRegionDictionary("Cr1", "Importer1", crisis1, configuration);
		regionDictionaryDAO.createRegionDictionary("Cr2", "Importer1", crisis2, configuration);
		...
		final RegionDictionary noEntityRD = new RegionDictionary("Another unnormalized name", "Another Importer", null, configuration);
		...
		regionDictionaryDAO.createRegionDictionary(noEntityRD.getId().getUnnormalizedName(), noEntityRD.getId().getImporter(), noEntityRD.getEntity(), configuration);
		...
		regionDictionaryDAO.createRegionDictionary(regionDictionaryToDelete.getId().getUnnormalizedName(), regionDictionaryToDelete.getId().getImporter(), regionDictionaryToDelete.getEntity(), configuration);
		regionDictionaryList = regionDictionaryDAO.listRegionDictionaries(null);
		assertEquals("region_dictionary table should now have 5 entries", 5, regionDictionaryList.size());
