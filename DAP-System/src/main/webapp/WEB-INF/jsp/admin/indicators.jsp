...
<body>
	...
	<form method="POST" action="">
		<label for="sourceCode">Source</label> <select name="sourceCode" id="sourceCode">
			<c:forEach var="source" items="${it.sources}">
				<option value="${source.code}">${source.name}</option>
			</c:forEach>
		</select> <label for="entityId">Entity</label> <select name="entityId" id="entityId">
			<c:forEach var="entity" items="${it.entities}">
				<option value="${entity.id}">${entity.name}(${entity.type.name})</option>
			</c:forEach>
		</select> <label for="indicatorTypeCode">Indicator type</label> <select name="indicatorTypeCode" id="indicatorTypeCode">
			<c:forEach var="indicatorType" items="${it.indicatorTypes}">
				<option value="${indicatorType.code}">${indicatorType.name}</option>
			</c:forEach>
		</select> <br /> <label for="start">Start date (yyyy-mm-dd)</label> <input type="text" name="start" id="start" /> <label for="end">End
			date (yyyy-mm-dd)</label> <input type="text" name="end" id="end" /> <label for="periodicity">Periodicity</label> <select name="periodicity"
			id="periodicity">
			<c:forEach var="periodicity" items="${it.periodicities}">
				<option value="${periodicity}">${periodicity}</option>
			</c:forEach>
		</select> <label for="numeric">Is numeric</label> <input type="checkbox" name="numeric" value="true"> <br /> <label for="value">Value</label>
		<input type="text" name="value" id="value" /> <label for="initialValue">Initial value</label> <input type="text" name="initialValue"
			id="initialValue" /> <input type="submit" value="submit" />
